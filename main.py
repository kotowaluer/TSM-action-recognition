...
def main():
    ...
    for group in policies:
        print(
            f"group: {group['name']} has {len(group['params'])} params, lr_mult: {group['lr_mult']}, decay_mult: {group['decay_mult']}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            prec1, val_loss, cm = validate(val_loader, model, criterion, epoch, log_training, tf_writer)
            ...
            if is_best:
                best_prec1 = prec1
                best_confusion_matrix = cm  # 仅保存最佳精度的混淆矩阵
            ...
            if optimizer.param_groups[-1]['lr'] <= 0:
                optimizer.param_groups[-1]['lr'] = 0.00001
        # 在训练结束后保存最终的最佳混淆矩阵
    if best_confusion_matrix is not None:
        plot_confusion_matrix_to_tensorboard(best_confusion_matrix, args.epochs, tf_writer)
        print("Best confusion matrix saved to TensorBoard.")
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            all_preds.append(torch.argmax(output, dim=1).cpu().numpy())
...
def plot_confusion_matrix_to_tensorboard(cm, epoch, writer):
    """将混淆矩阵绘制为图像，并写入 TensorBoard"""
    ...
    writer.add_figure(f'Confusion_Matrix/best', fig)
    plt.close(fig)
