...
class TSN_With_Hand_LSTM(nn.Module):
    def __init__(self, num_class, num_segments, modality,
        ...
        super(TSN_With_Hand_LSTM, self).__init__()
    ...
    def _prepare_tsn(self, num_class):
        feature_dim = getattr(self.base_model, self.base_model.last_layer_name).in_features + 4
        if self.dropout == 0:
            ...
            if self.is_shift:
                ...
                for m in self.base_model.modules():
                    if isinstance(m, InvertedResidual) and len(m.conv) == 8 and m.use_res_connect:
                        if self.print_spec:
                            print(f'Adding temporal shift... {m.use_res_connect}')
    ...
    def train(self, mode=True):
        ...
        :return:
        """
        super(TSN_With_Hand_LSTM, self).train(mode)
        count = 0
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d) or isinstance(m, torch.nn.Conv3d):
                ...
                for name, param in m.named_parameters():
                    if 'weight' in name:
                        lstm_weights.append(param)
                    elif 'bias' in name:
                        lstm_biases.append(param)
    ...
    def forward(self, input, hand_info, no_reshape=False):
        if not no_reshape:
        ...
        hand_info = hand_info.view(-1, 4)
        base_out = torch.cat((base_out, hand_info), dim=1)
